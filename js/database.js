...
tv.indexedDB.updateEpisode = function(data){
	var db = tv.indexedDB.db;
	var trans = db.transaction([DB_epi],"readwrite");
	var store = trans.objectStore(DB_epi);
	var range = IDBKeyRange.only(data.episodeID);
	var cursor = store.openCursor(range);
	cursor.onsuccess = function(e){
		var cur = e.target.result;
		var req = cur.update(data);
		req.onsuccess = function(){
			bootbox.alert("Episode updated.");
		};
		req.onerror = function(){
			bootbox.alert("Error updating episode.");
		};
	};
